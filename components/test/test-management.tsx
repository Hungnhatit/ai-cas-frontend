    ...
    <div className="">
      ...
      <Tabs defaultValue="all">
                ...
                <Card key={test.ma_kiem_tra} className="hover:shadow-lg transition-shadow">
                  <CardHeader>
                    <div className="flex items-start justify-between mb-2">
                      ...
                      <span className={`px-2 py-1 rounded-full text-xs font-medium ${test.trang_thai === 'active' ? 'bg-green-100 text-green-700' :
                        test.trang_thai === 'draft' ? 'bg-yellow-100 text-yellow-700' :
                  ...
                  {tests.map(test => (
                    <tr key={test.ma_kiem_tra} className="hover:bg-gray-50">
                      <td className="px-6 py-4">
                        <div className="font-medium text-gray-900">{test.tieu_de}</div>
                        <div className="text-sm text-gray-500">{test.do_kho}</div>
                      </td>
                      <td className="px-6 py-4">
                        <span className={`px-2 py-1 rounded-full text-xs font-medium ${test.trang_thai === 'active' ? 'bg-green-100 text-green-700' :
                          test.trang_thai === 'draft' ? 'bg-yellow-100 text-yellow-700' :
        ...
        <TabsContent value="active" className="mt-6">
          <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6">
            {tests.filter(e => e.trang_thai === 'active').map(test => (
        ...
        <TabsContent value="draft" className="mt-6">
          <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6">
            {tests.filter(e => e.trang_thai === 'draft').map(test => (
              <Card key={test.ma_kiem_tra}>
                <CardHeader>
                  <CardTitle>{test.tieu_de}</CardTitle>
                </CardHeader>
